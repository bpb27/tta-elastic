    ...
    return (
      ...
      >
        ...
        <div className={styles.buttons}>
          <Button
            onClick={() => this.changePage('retribution')}
            selected={section === 'retribution'}
          >
            Retribution
          </Button>
          <Button onClick={() => this.changePage('women')} selected={section === 'women'}>
            Women
          </Button>
          <Button onClick={() => this.changePage('obama')} selected={section === 'obama'}>
            Obama
          </Button>
          <Button onClick={() => this.changePage('attacks')} selected={section === 'attacks'}>
            Some Attacks
          </Button>
          <Button onClick={() => this.changePage('assorted')} selected={section === 'assorted'}>
            Assorted
          </Button>
        </div>
        <EmbeddedTweetColumn>
          {this.list.map(data => (
            <TweetLink key={data.id} placeholderAlignment="center" type="embed" tweetData={data} />
          ))}
        </EmbeddedTweetColumn>
      </Page>
    );
